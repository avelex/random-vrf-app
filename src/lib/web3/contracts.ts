	...
	try {
		...
		try {
			...
			try {
				...
				if (logs.length > 0) {
					console.log('Step 1: RandomnessRequested event found in transaction');
					...
					// Get the current block number from Arbitrum Sepolia
					const currentBlockNumber = await publicClient.getBlockNumber();
					console.log('Current Block number (for fake request):', currentBlockNumber);
...
async function listenForRandomnessReceived(requestId: string) {
	try {
		console.log(
			'Step 4: Setting up listener for RandomnessReceived event for requestId:',
			requestId
		);
...
async function pollForRandomnessReceivedEvent(requestId: string, network: NetworkConfig) {
	...
	const pollInterval = setInterval(async () => {
		...
		try {
			...
			if (logs.length > 0) {
				console.log('Found RandomnessReceived event in transaction:', logs[0].transactionHash);
...
async function listenForRandomNetworkEvents(requestId: string) {
	try {
		console.log(
			'Steps 2-3: Setting up listeners for Random Network events for requestId:',
			requestId
		);
		...
		randomVrfCoreContract.on(requestReceivedFilter, (event: any) => {
			...
			console.log('Step 2: RequestReceived event detected in transaction:', txHash);
		...
		randomVrfCoreContract.on(vrfExecutedFilter, (event: any) => {
			...
			console.log('Step 3: VRFExecuted event detected in transaction:', txHash);
		...
		if (pastVrfExecutedEvents.length > 0) {
			...
			console.log(
				'Step 3 completed (from past events), now waiting for Step 4 (RandomnessReceived event)'
			);
		}
	} catch (error) {
